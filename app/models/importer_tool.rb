class ImporterTool < ActiveRecord::Base
  class << self
    ...
    def find_contact_hash_from(import_source, import_hash )
      if import_source == "custom"
        custom_contact_hash(import_hash)
      elsif import_source == "google"
        google_contact_hash(import_hash)
    ...
    def find_address_hash_from(import_source, import_hash, address_kind)
      ...
      if import_source == "custom"
        ...
        custom_address_hash(import_hash,address_kind, us_state, country)
        ...
        country_code = "US" #refactor
        country = Country.find_by_code(country_code) || Country.find_by_code("US")
        google_address_hash(import_hash, address_kind, us_state, country)
  ...
  #       Phone,Home Phone,Home Phone 2,Mobile Phone,Pager,Home Fax,Home Address,Home Street,Home Street 2,Home Street 3,Home Address PO Box,Home City,Home State,Home Postal Code,Home Country,Spou
   ...
   def custom_contact_hash(import_hash)
      {
   ...
   def google_contact_hash(import_hash)
         {
            first_name: import_hash["First Name"],
            middle_name: import_hash["First Name"],
            last_name: import_hash["Last Name"],
    ...
    def google_address_hash(import_hash, address_kind, us_state_id, country)
         {
    ...
    def custom_address_hash(import_hash, address_kind, us_state_id, country)
         {
          title: address_kind.titleize,
